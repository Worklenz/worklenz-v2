...
export const on_custom_column_pinned_change = async (io: Server, socket: Socket, data: string) => {
  try {
    ...
    if (!column_id || !project_id || is_visible === undefined) {
      console.error('Invalid data for custom column pinned change', { column_id, project_id, is_visible });
      return;
    }
    ...
    if (result.rowCount === 0) {
      console.error('Custom column not found or not updated', { column_id, project_id });
      return;
    }
    ...
    console.log('Custom column visibility updated successfully', { column_id, is_visible });
  } catch (error) {
    log_error(error);
    console.error('Error updating custom column visibility', error);
  }
};